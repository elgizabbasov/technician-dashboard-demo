<template>
	<v-container fluid>
		<v-card width="300px">
			...
			<v-container v-if="isSensorDataEmpty">
				<span style="font-size: 18px; color: black">
					No previous sensor health data found!
				</span>
			</v-container>
			<v-container v-else fluid>
				<v-list>
					<v-list-item v-for="sensor in sensors" :key="sensor.type">
		...
		addSensors() {
		  setTimeout(() => {
				...
				for (let i = 0; i < this.$store.getters.pSensorLength; i++) {
					...
					if (
						Math.abs(Date.now() - Date.parse(array[j]) >= 86400000)
					) {
						this.sensors.push({
							type: 'Pressure ' + j.toString(),
							health: 'Bad',
							color: 'red',
						});
					} else if (
						Math.abs(Date.now() - Date.parse(array[j])) >= 3600000
					) {
						// yellow if more than 1 hr
						this.sensors.push({
							type: 'Pressure ' + j.toString(),
							health: 'Needs Review',
							color: 'yellow',
